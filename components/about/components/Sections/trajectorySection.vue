<template>
  <div class="bg-image">
    <div class="bg-black bg-opacity-50 py-16">
      <div
        class="grid grid-cols-1 sm:grid-cols-2 gap-4 section-container container mx-auto my-15 text-slate-50"
      >
        <div>
          <div class="text-[48px] font-black">El legado del FICA</div>
          <div class="text-base font-light mb-6">
            "La historia de Pepe Peña y el impacto del festival"
          </div>
          <div class="text-base font-bold">
            Con Pepe Peña como primer ganador y su emblemático tema "Ribereña",
            el FICA se posicionó como el segundo mejor festival de América, solo
            después de Viña del Mar. A lo largo de sus diez ediciones, el FICA
            se convirtió en un evento de gran importancia para la promoción
            artística, la creación cultural y la manifestación pura de la
            Amazonía. Además de destacar los paisajes, la biodiversidad y el
            turismo como motor de desarrollo en la región.
          </div>
        </div>
        <div class="flex items-center justify-center">
          <img src="/assets/images/pepe_pe.png" alt="pepe_peña" />
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.bg-image {
  background-image: url("/assets/images/legado_fica.png");
  background-size: cover;
  background-position: start;
}
</style>
